...
\subsection{Release contabile e file batch}

La release CBR e collocata in:

\begin{lstlisting}
Release\release_contabilita_CBR
\end{lstlisting}
...
\begin{enumerate}
  ...
  \item imposta la sorgente DBF CBR;
  \item imposta la cartella di esportazione;
  \item avvia \texttt{Crea\_Report.exe} con il config contabile;
  \item salva output ed errori in un file di log con data e ora;
  \item restituisce il codice di uscita dell'eseguibile.
\end{enumerate}

Per eseguire la release e sufficiente avviare:

\begin{lstlisting}
Contabilita\run_contabilita.bat
\end{lstlisting}
...
cartella \texttt{Contabilita/logs}.
